...
function LayerRow({
    ...
    return (
        ...
        >
            <button type="button" onClick={onActivate} className="flex min-w-0 flex-1 items-center gap-2 py-1 text-left">
                ...
                {configs.map((_, i) => {
                    ...
                    return (
                        <button
                            ...
                            style={isSelected ? undefined : { borderColor: `${keyframeColor(i)}99`, color: keyframeColor(i) }}
                        >
                            {i + 1}
                        </button>
                    );
...
export default function Playground() {
        ...
        const updateConfig = (edge: 'wave' | 'top' | 'bottom', i: number, config: WaveConfig) =>
            setLayers((ls) =>
                ls.map((l) => {
                    ...
                    return { ...l, [edge]: { ...l[edge], configs: l[edge].configs.map((c, j) => (j === i ? config : c)) } };
                }),
            );
        if (active.mode === 'wave') {
            return [
    ...
    const buildExport = (): { svg: string; t: number } | null => {
        ...
        const svg = buildFigmaSvg(visible, rect.width, rect.height, (layer) =>
            layerProgress(layer.scrollStart, layer.scrollEnd, sectionTop, rect.height, window.innerHeight, window.scrollY),
        );
    ...
    // Escape releases the pinned keyframe.
    useEffect(() => {
        ...
        <div className="relative min-h-screen max-w-full overflow-x-hidden bg-gray-darkest-washed text-white-washed">
            ...
            <main className="lg:pr-[380px]">
                <div className="flex h-[80vh] flex-col items-start justify-center gap-2 px-4 pb-8 md:px-32 xl:px-56">
                    <h1 className="text-4xl font-bold">Wave Composer</h1>
                    <p className="max-w-sm text-sm opacity-80">
                        Stack waves and bands as layers, edit each one right on the canvas — and copy the whole
                        composition as SVG into Figma at any scroll position.
                    </p>
                </div>
                ...
                <div className="flex h-[80vh] items-end px-4 pb-16 md:px-32 xl:px-56">
                    <p className="max-w-md font-mono text-xs leading-relaxed opacity-60">
            ...
            >
                {/* always visible: layer palette + scroll scrubber + figma export */}
                    ...
                    <>
                        ...
                        {selection && selectedConfigs ? (
                            ...
                            <p className="text-xs leading-relaxed opacity-70">
                                Pick a keyframe chip to pin and edit it — drag the anchors and handles on the canvas.
                                Onion skin shows all keyframes as dashed system curves (clickable too). 1 keyframe is
                                static, 2 blend linearly, 3+ flow through a smooth spline.
                            </p>
                        )}
                    </>,
                )}
                            ...
                            <div className="mt-2 grid grid-cols-2 gap-2">
                                {(['scrollStart', 'scrollEnd'] as const).map((key) => (
                                    <label key={key} className="flex flex-col gap-1">
                                        <span className="opacity-80">{key === 'scrollStart' ? 'start' : 'end'}</span>
                                        <input
                                            type="text"
                                            value={active[key]}
